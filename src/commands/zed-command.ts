...
export function registerZedCommands(pi: ExtensionAPI): void {
  const handlers: Record<string, (args: string, ctx: ExtensionCommandContext) => Promise<void>> = {
    usage: async (_a, ctx) => {
      ctx.ui.notify("Fetching live usage from Zed Cloud...", "info");
      const creds = loadCredentials();
      if (!creds || (!creds.accessToken && !creds.sessionCookie)) { ctx.ui.notify("No Zed credentials found. Please run '/zed login' to connect your Zed account.", "warning"); return; }
      const report = await fetchZedUsage(creds);
      if (report?.username && !creds.githubUsername) saveCredentials({ ...creds, githubUsername: report.username, userId: report.userId || creds.userId });
      ctx.ui.notify(formatUsageSummary(report), "info");
    },
    status: async (_a, ctx) => {
      const creds = loadCredentials();
      if (!creds || (!creds.accessToken && !creds.sessionCookie)) { ctx.ui.notify("Zed Status: Disconnected (No credentials stored). Run '/zed login'.", "warning"); return; }
    ...
    login: async (_a, ctx) => {
      ...
      try { const c = await startOAuthFlow(); ctx.ui.notify(`✓ Successfully logged in as ${c.githubUsername || "Zed User"}!`, "info"); }
      catch (e) { ctx.ui.notify(`Authentication failed: ${e instanceof Error ? e.message : String(e)}`, "error"); }
    },
    logout: async (_a, ctx) => {
    ...
    "set-token": async (args, ctx) => {
      ...
      if (!token) { ctx.ui.notify("Usage: /zed set-token <your_token>", "warning"); return; }
      saveCredentials({ accessToken: token }); ctx.ui.notify("✓ Token saved successfully!", "info");
    },
    "set-cookie": async (args, ctx) => {
      const cookie = args.replace(/^set-cookie\s+/i, "").trim();
      if (!cookie) { ctx.ui.notify("Usage: /zed set-cookie <your_session_cookie>", "warning"); return; }
      saveCredentials({ sessionCookie: cookie }); ctx.ui.notify("✓ Session cookie saved successfully! Live dashboard sync enabled.", "info");
    },
    "set-spend": async (args, ctx) => {
      const amount = parseFloat(args.replace(/^set-spend\s+/i, "").replace(/^\$/, "").trim());
      if (isNaN(amount) || amount < 0) { ctx.ui.notify("Usage: /zed set-spend <amount_in_usd> (e.g. /zed set-spend 0.53)", "warning"); return; }
      setLocalSpendAmount(amount); ctx.ui.notify(`✓ Current spend set to $${amount.toFixed(2)}.`, "info");
    },
    sync: async (_a, ctx) => {
