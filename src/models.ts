 ...
 */
export const DEFAULT_MODEL_PRICING: Record<string, ModelPricingEntry> = {
...
const FALLBACK_PRICING: ModelPricingEntry = DEFAULT_MODEL_PRICING["claude-sonnet-4-6"]!;

export function mapRawModelToConfig(raw: ZedRawModel): ProviderModelConfig {
...
export function loadCachedZedModels(): ProviderModelConfig[] | null {
  const p = path.join(getOmpAgentDir(), "zed_models_cache.json");
  try {
    if (fs.existsSync(p)) {
      const data = JSON.parse(fs.readFileSync(p, "utf-8")) as { models: ProviderModelConfig[] };
...
export function saveCachedZedModels(models: ProviderModelConfig[]): void {
  try {
...
export function getCachedZedModels(): ProviderModelConfig[] {
  return loadCachedZedModels() || ZED_MODELS;
}

export async function fetchRemoteZedModels(creds?: ZedCredentials | null): Promise<ProviderModelConfig[]> {
...
export async function getZedModels(creds?: ZedCredentials | null, forceRefresh = false): Promise<ProviderModelConfig[]> {
  if (!forceRefresh) {
